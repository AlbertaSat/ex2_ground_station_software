...
hkCommands = ["GET_HK", "GET_INSTANT_HK", "GET_LATEST_HK"]
# List of HK commands that need a special handler
...
class InteractiveHandler:
    ...
    def getTransactionObject(self, command: str, networkHandler):
        ...
        if self.dummy:
            ...
            tokens[self.serviceIdx] == "HOUSEKEEPING"
            and tokens[self.subserviceIdx] in hkCommands
        ):
        ...
        elif tokens[self.serviceIdx] == "SCHEDULER" and (
            tokens[self.subserviceIdx] == "SET_SCHEDULE"
        ):
            return schedulerTransaction(command, networkHandler)
        elif (
            tokens[self.serviceIdx] == "TIME_MANAGEMENT"
            and tokens[self.subserviceIdx] == "SET_TIME"
        ):
            return setTimeTransaction(command, networkHandler)
        elif tokens[self.serviceIdx] == "IRIS" and (
            tokens[self.subserviceIdx] in ["IRIS_SET_TIME"]
        ):
        ...
        elif (tokens[self.serviceIdx] == "GENERAL") and (
            tokens[self.subserviceIdx == "DEPLOY_DEPLOYABLES"]
        ):
            return longTimeoutTransaction(command, networkHandler)
        elif tokens[self.serviceIdx] == "COMMUNICATION" and (
            tokens[self.subserviceIdx] in ["UHF_SET_RF_MODE"]
        ):
    ...
    def getDummyTransactionObject(self, command: str, networkHandler):
        ...
        if (
            ...
            tokens[self.subserviceIdx] in ["SET_SCHEDULE", "DELETE_SCHEDULE"]
        ):
...
class dummyTransaction(baseTransaction):
    def execute(self):
        return {"dst": self.dst, "dport": self.dport, "args": self.args}
...
class dummySchedulerTransaction(baseTransaction):
    def execute(self):
        ...
        packetEmbedder = EmbedPacket(cmdList, self.args)
        self.args = packetEmbedder.embedCSP()
        return {"err": 0, "dst": self.dst, "dport": self.dport, "args": self.args}
...
class setRFModeTransaction(baseTransaction):
    ...
    def execute(self):
        self.send()
        print(
            "Command does not give valid return upon success. Need to wait >20 seconds for Pipe mode to time out"
        )
        time.sleep(
            21
        )  # TODO make coupled with uTransceiver pipemode time in case of change
