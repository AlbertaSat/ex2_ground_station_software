...
class ReceiveParser:
    ...
    def parseReturnValue(self, src, dport, data):
        ...
        # TODO: Refactor this
        length = len(data)
        service = [x for x in services if services[x]["port"] == dport][0]

        idx = 0
        outputObj = {}
        subservice = {}

        if service and ("subservice" in services[service]) and length > 0:
            subservice = [
                services[service]["subservice"][x]
                for x in services[service]["subservice"]
                if services[service]["subservice"][x]["subPort"] == data[idx]
            ][0]
            idx += 1
            ...
            if returns[retVal] == "var":
                ...
                try:
                    outputObj[retVal] = np.frombuffer(
                        data, dtype=returns[retVal], count=1, offset=idx
                    )[0]
                except:
...
if __name__ == "__main__":
    ...
    returnval = parser.parseReturnValue(
        8, bytearray(b"\x01\x00")
    )  # ba[0] = 01 (set time)
    print(returnval)

    returnval = parser.parseReturnValue(
        8, bytearray(b"\x00\x00@\xaa\xe1H@\x06ffA\x90\x14{")
    )  # ba[0] = 00 (set time)
    print(returnval)
