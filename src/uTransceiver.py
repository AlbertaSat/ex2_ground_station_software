    ...
    def UHFDIRCommand(self, string):
        ...
        if cmdcode == 0:
            ...
            param = ctypes.c_uint16(
                param
            )  # TODO check that this works with space after comma in command
        ...
        if (time.time() - self.last_tx_time) > self.pipetimeout_s:
            ...
            time.sleep(
                0.2
            )  # to prevent csp packet transmission interrupting UHFDIR command transmission (sleeping length not yet optimized)
