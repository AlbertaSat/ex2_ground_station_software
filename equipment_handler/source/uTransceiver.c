...
UHF_return UHF_genericRead(uint8_t code, void *param) {
  ...
  if (code == 253) {
    ...
    uint8_t fram_command[MAX_R_CMDLEN] = {'E',         'S',
                                          '+',         'R',
                                          '2',         sm_add,
                                          'F',         'D',
                                          chadd[0],    chadd[1],
  ...
  switch (code) {
    ...
    case 1: {  // Get the frequency (Unsure if this is correct-> index 5 of hex
      ...
      uint32_t val2 = (hex[4] << 20) | (hex[5] << 16) | (hex[2] << 12) |
                      (hex[3] << 8) | (hex[0] << 4) | hex[1];
    ...
    case 8:  // Get Audio Beacon period
    {
      ...
      *value = (hex[0] << 28) | (hex[1] << 24) | (hex[2] << 20) |
               (hex[3] << 16) | (hex[4] << 12) | (hex[5] << 8) | (hex[6] << 4) |
               hex[7];
    ...
    case 255: {  // Secure Mode read
      ...
      uint8_t hex[8] = {ans[3], ans[4], ans[5], ans[6],
                        ans[7], ans[8], ans[9], ans[10]};
      convHexFromASCII(8, hex);
      *key = (hex[0] << 28) | (hex[1] << 24) | (hex[2] << 20) | (hex[3] << 16) |
             (hex[4] << 12) | (hex[5] << 8) | (hex[6] << 4) | hex[7];
...
UHF_return UHF_genericI2C(uint8_t format, uint8_t s_address, uint8_t len,
  ...
  uint8_t cmd[MAX_W_CMDLEN] = {
      'E', 'S',    '+',       'W',       '2',       sm_add,   'F',
      '1', format, params[0], params[1], params[2], params[3]};
  int i = 0;
