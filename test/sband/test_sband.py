...
def testAllCommandsToOBC():
    ...
    tester.sendAndExpect('ex2.communication.s_get_full_status', {'err': 0, 'PWRGD': 1, 'TXL': 1, 'Transmit Ready': 1, 'Buffer Count': 0, 'Buffer Underrun': 1, 'Buffer Overrun': 2, 'Output Power': np.float32(26), 'Power Amplifier Temperature': np.float32(
    ...
    tester.sendAndExpect('ex2.communication.s_set_freq(2250.5)', {'err': 0})
    tester.sendAndExpect('ex2.communication.s_get_freq', {
    ...
    tester.sendAndExpect('ex2.communication.s_set_encoder(1 1 0 1)', {'err': 0})
    tester.sendAndExpect('ex2.communication.s_get_encoder', {
                  'err': 0, 'scrambler': 1, 'filter': 1, 'modulation': 0, 'rate': 1})
    tester.sendAndExpect('ex2.communication.s_get_status',
                  {'err': 0, 'PWRGD': 1, 'TXL': 1})
    tester.sendAndExpect('ex2.communication.s_get_TR', {
                  'err': 0, 'Transmit Ready': 1})
    tester.sendAndExpect('ex2.communication.s_get_buffer(0)', {'err': 0, 'buffer': 0})
    tester.sendAndExpect('ex2.communication.s_get_buffer(1)', {'err': 0, 'buffer': 1})
    tester.sendAndExpect('ex2.communication.s_get_buffer(2)', {'err': 0, 'buffer': 2})
    tester.sendAndExpect('ex2.communication.s_get_hk', {'err': 0, 'Output Power': np.float32(26), 'Power Amplifier Temperature': np.float32(27.3), 'Top Temperature': np.float32(-2.8), 'Bottom Temperature': np.float32(
    ...
    tester.sendAndExpect(
        'ex2.communication.s_set_config(2222.5 28 0 0 1 1 1 0)', {'err': 0})
    tester.sendAndExpect('ex2.communication.s_get_config', {'err': 0, 'Frequency': np.float32(2222.5), 'Power Amplifier Power': 28, 'Power Amplifier status': 0,
                                                     'Power Amplifier mode': 0, 'Encoder scrambler': 1, 'Encoder filter': 1, 'Encoder modulation': 1, 'Encoder rate': 0})
    # tester.sendAndExpect('ex2.communication.s_soft_reset', {'err': 0})
