...
def testAllCommandsToOBC():
    ...
    test.send(
        "obc.dfgm.dfgm_get_hk"
    )  # Update empty HK buffer by turning on DFGM for 1 sec
    time.sleep(2)

    test.send(
        "obc.dfgm.dfgm_get_hk"
    )  # Should instantly get updated data from the HK buffer
    ...
    time.sleep(1)
    test.sendAndExpect(
        "obc.dfgm.dfgm_stop", {"err": 0}
    )  # Check if DFGM actually stops dfgm_run
    ...
    test.sendAndExpect(
        "obc.dfgm.dfgm_stop", {"err": 0}
    )  # Check if DFGM actually stops dfgm_start

    test.send(
        "obc.dfgm.dfgm_get_hk"
    )  # Check if data gets corrupt somehow (temp. should be ~20, not ~65000)

    # Error checking tests
    test.sendAndExpect(
        "obc.dfgm.dfgm_run(2147483648)", {"err": 1}
    )  # Bad param. - Overflow for 32-bit int
    test.sendAndExpect(
        "obc.dfgm.dfgm_run(0)", {"err": 1}
    )  # Bad param. - Less than minRuntime
    ...
    test.send(
        "obc.dfgm.dfgm_get_hk"
    )  # See if any of the values get updated from other packets
    test.sendAndExpect("obc.dfgm.dfgm_stop", {"err": 0})

    test.sendAndExpect(
        "obc.dfgm.dfgm_stop", {"err": 0}
    )  # Should not fail if nothing is running
